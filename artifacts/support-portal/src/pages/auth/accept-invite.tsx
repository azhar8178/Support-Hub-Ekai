...
export default function AcceptInvitePage() {
    ...
    return (
      <div className="flex min-h-[100dvh] items-center justify-center bg-slate-50 px-4">
        <div className="w-full max-w-md bg-white rounded-2xl p-8 text-center border border-slate-200 shadow-sm">
          ...
          <p className="text-slate-600 mb-6">No invitation token was found in the URL.</p>
  ...
  if (isPreviewLoading) {
    return (
      <div className="flex min-h-[100dvh] items-center justify-center bg-slate-50 px-4">
        <div className="w-full max-w-md bg-white rounded-2xl p-12 text-center border border-slate-200 shadow-sm flex flex-col items-center">
    ...
    return (
      <div className="flex min-h-[100dvh] items-center justify-center bg-slate-50 px-4 py-12">
        <div className="w-full max-w-md bg-white rounded-2xl p-8 border border-slate-200 shadow-sm text-center">
          <div className="mx-auto w-16 h-16 bg-blue-50 rounded-full flex items-center justify-center mb-6 border border-blue-100">
          ...
          <div className="bg-slate-50 p-4 rounded-lg border border-slate-100 text-left mb-8">
            <p className="text-xs text-slate-500 font-medium uppercase tracking-wider mb-2">Invitation Details</p>
          ...
          <div className="space-y-3">
            <Button onClick={() => setLocation("/sign-up")} className="w-full bg-[#2563EB] hover:bg-[#1d4ed8] h-11">
              Create Account <ArrowRight className="ml-2 h-4 w-4" />
            </Button>
            <p className="text-sm text-slate-500">
              Already have an account? <Link href="/sign-in" className="text-[#2563EB] font-medium hover:underline">Sign in</Link>
            </p>
          </div>
        </div>
      </div>
  ...
  return (
    <div className="flex min-h-[100dvh] items-center justify-center bg-slate-50 px-4 py-12">
      <div className="w-full max-w-md bg-white rounded-2xl p-10 text-center border border-slate-200 shadow-sm flex flex-col items-center">
        {acceptError ? (
          <>
            <div className="mx-auto w-12 h-12 bg-red-100 rounded-full flex items-center justify-center mb-6">
              <ShieldAlert className="h-6 w-6 text-red-600" />
            ...
            <p className="text-slate-600 mb-6 text-sm">{acceptError}</p>
            <Button onClick={handleAccept} disabled={isAccepting} className="w-full bg-[#2563EB]">
          ...
          <>
            <div className="mx-auto w-16 h-16 bg-blue-50 rounded-full flex items-center justify-center mb-6 relative">
              <Loader2 className="h-10 w-10 animate-spin text-[#2563EB] absolute" />
              <CheckCircle2 className="h-5 w-5 text-blue-700" />
            </div>
            <h1 className="text-xl font-bold text-[#0F1F3D] mb-2">Setting up your access...</h1>
            <p className="text-slate-500 text-sm">Please wait while we link your account to {preview?.orgName || 'the portal'}.</p>
          </>
        )}
      </div>
    </div>
