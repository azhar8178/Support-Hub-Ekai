...
export default function DashboardPage() {
  ...
  if (isLoading) {
    return (
      <div className="p-8">
        <div className="h-8 w-64 bg-stone-200 rounded animate-pulse mb-8"></div>
    ...
    <div className="p-8 max-w-7xl mx-auto">
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-stone-500 mt-1">Overview of your support tickets and recent activity.</p>
        </div>
        <Button asChild className="bg-[#EFB323] hover:bg-[#D69E1E]">
          ...
          <CardContent className="p-6">
            <div className="flex items-center space-x-4">
              <div className="p-3 bg-amber-50 rounded-full">
                <TicketIcon className="h-6 w-6 text-[#EFB323]" />
              </div>
              <div>
                <p className="text-sm font-medium text-stone-500">Open Tickets</p>
        ...
        <Card>
          <CardContent className="p-6">
            <div className="flex items-center space-x-4">
              ...
              <div>
                <p className="text-sm font-medium text-stone-500">Resolved (30d)</p>
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between">
          ...
          <Button variant="link" className="text-[#B45309]" asChild>
        ...
        <CardContent>
          {summary.recentTickets.length === 0 ? (
            <div className="text-center py-12">
              <TicketIcon className="mx-auto h-12 w-12 text-stone-300 mb-4" />
              <h3 className="text-lg font-medium text-[#0F1F3D]">No recent tickets</h3>
              <p className="text-stone-500 mt-1 mb-6">You don't have any recent support tickets.</p>
              ...
              <Table>
                ...
                <TableBody>
                  {summary.recentTickets.map((ticket) => (
                    <TableRow key={ticket.id} className="cursor-pointer hover:bg-stone-50 transition-colors" onClick={() => window.location.href = `/tickets/${ticket.id}`}>
                      <TableCell>
                        <div className="font-medium text-[#0F1F3D]">{ticket.title}</div>
                        <div className="text-xs text-stone-500">#{ticket.id} • {ticket.category}</div>
                      </TableCell>
                      <TableCell><SeverityBadge severity={ticket.severity} /></TableCell>
                      <TableCell><StatusBadge status={ticket.status} /></TableCell>
                      <TableCell>
                        <span className="capitalize text-sm text-stone-600">{ticket.environment}</span>
                      </TableCell>
                      <TableCell>
